...
case class DeployTaskForCI() extends DeployTask {
  ...
  override def handleResponse(metadataConnection: MetadataConnection, response: SFDCMDAPIAntTask.StatusResult): Unit = {
    ...
    val deployResult = metadataConnection.checkDeployStatus(response.getId(), true);
    var details = deployResult.getDetails();
